...
def parseArguments(argvs):
    ...
    for i in range(1, len(argvs)):
        ...
        if argvs[i] == "--scriptfolder" or argvs[i] == "-f":
            parameter["scripts"] = argvs[i + 1]
    return parameter
...
def runGOATOOLS(params, motif_type, modules, go_associations, rID, loadType, p_value, scripts):
    """
...
if __name__ == '__main__':
    ...
    for i in split.keys():
        ...
        # get background
        if param["annotation"] == "ModuleType": 
            runGOATOOLS(param, i, Modules, param["GO_associations"], rID, i, float(param["pvalue"]), param["scripts"])
        else:
            runGOATOOLS(param, i, Modules, param["GO_associations"], rID, "full", float(param["pvalue"]), param["scripts"])
